...
def user_name(user: dict) -> str:
    return str(
        field(user, "name", "fullName", "firstName", "email", default=user_id(user) or "Unassigned")
    )
...
def opportunity_stage_id(opportunity: dict) -> str:
    ...
    return str(field(opportunity, "pipelineStageId", "stageId", default=""))
...
def opportunity_stage_name(opportunity: dict, pipelines: list) -> str:
    ...
    for pipeline in pipelines:
        for pipeline_stage in pipeline.get("stages", []) or []:
            if str(field(pipeline_stage, "id", "_id", default="")) == stage_id:
                return str(field(pipeline_stage, "name", "title", default=stage_id))
...
def contact_name(record: dict) -> str:
    ...
    if isinstance(contact, dict):
        ...
        if name:
            return str(name)
    return str(field(record, "contactName", "name", "fullName", "title", default="Unknown"))
...
def compact_lead(record: dict, pipelines: list | None = None) -> dict:
    ...
    if pipelines is not None:
        item["stage"] = opportunity_stage_name(record, pipelines)
    return item
...
def hot_leads(data: dict) -> list:
    ...
    for opportunity in data["opportunities"]:
        stage_name = opportunity_stage_name(opportunity, data["pipelines"]).lower()
        status = str(field(opportunity, "status", default="")).lower()
        value = field(opportunity, "monetaryValue", "value", default=0) or 0
        try:
            value = float(value)
        except (TypeError, ValueError):
            value = 0
        ...
        has_waiting_message = any(is_inbound_waiting(item) for item in related_conversations)
        is_hot_stage = any(word in stage_name for word in ("hot", "qualified", "quote", "appointment", "ready"))

        if status not in {"lost", "won", "closed"} and (is_hot_stage or value >= 1000 or has_waiting_message):
            item = compact_lead(opportunity, data["pipelines"])
            item["reason"] = {
                "hot_stage": is_hot_stage,
                "high_value": value >= 1000,
                "waiting_message": has_waiting_message,
            }
            hot.append(item)
...
def unattended_leads(data: dict) -> list:
    ...
    for opportunity in data["opportunities"]:
        status = str(field(opportunity, "status", default="")).lower()
        if status not in {"lost", "won", "closed"} and not assigned_user_id(opportunity):
            leads.append(compact_lead(opportunity, data["pipelines"]))
    return leads
...
def waiting_leads(data: dict) -> list:
    ...
    for conversation in data["conversations"]:
        ...
        if age > WAITING_THRESHOLD:
            item = compact_lead(conversation)
...
def agent_workload(data: dict) -> list:
    ...
    for opportunity in data["opportunities"]:
        ...
        status = str(field(opportunity, "status", default="")).lower()
        if status not in {"lost", "won", "closed"}:
